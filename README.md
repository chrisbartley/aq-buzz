...
Arduino-based sensing devices and an iOS app for the [Neosensory Buzz](https://neosensory.com/), enabling you to feel changes in the air quality around you.  
...
For the ease of putting something together quickly, both hardware and software, I went with Adafruit Feathers and breakout boards for the Arduino devices. Highly recommended! Thanks to Adafruit's' excellent documentation and example code, I had a first working version in about two days.  Simply based on my prior experience (I'd never used Arduinos before), I originally thought I'd go with a Raspberry Pi Zero + Node.js solution.  But the Adafruit Feather Arduinos won out in the end...they're just so much easier to get a prototype up and running quickly--both for me and for anyone wanting to replicate what I've done here.
...
Details such as actual parts lists, assembly instructions, and software support/usage are in separate documents:
...
Examples here fall under two primary device networking/communication topologies, but include beginnings and implementation notes (caveats!) for a third.

All communication with the Buzz is done via Bluetooth Low Energy (BLE).  All three topologies share the common features of the Buzz being a BLE peripheral and one or more Arduino-based devices + sensors doing the environmental sensing.  The topologies differ in which device is acting as the BLE central.  The three communication topologies presented here are:
...
This scenario consists of one or more Arduino devices as BLE peripherals, with an iOS app as BLE central.  The iOS app connects to all the Arduino peripherals it finds, as well as to the Buzz, and acts as a proxy between sensors and the Buzz, and is responsible for "translating" sensor data to Buzz vibrations.  The benefits of this model are:
...
To try it out, start with either of the device assembly + usage documents referenced above in the Hardware section.  I recommend the [Feather Sense Device](https://chrisbartley.github.io/aq-buzz/docs/feather-sense-assembly.html) since the instructions are a little more detailed and the software more fully fleshed out.